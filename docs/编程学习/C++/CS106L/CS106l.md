...
> [!tip] > `const`是保守的，分析是不精确的，它不是确定代码是否实际修改了`const`变量，而是检查：
> 是否对一个`const`变量赋值
...
> [!warning] > `throw;`必须用在`catch`块内部，否则程序会崩溃
...
> [!note] > `auto_ptr`  已被 C++11 的  `unique_ptr`  取代，因为它的所有权语义有缺陷（比如拷贝后原指针会变空）。
...
TL 的  `<functional>`  库用类似的技术实现了各种“适配器”和“可适配仿函数”。**adaptable function**：继承自  `unary_function`  或  `binary_function`，自动获得  `argument_type`  和  `result_type`  两个类型别名。
...
`binary_function`提供：`first_argument_type`第一个参数的类型，`second_argument_type`第二个参数的类型，`result_type`返回类型
...
拷贝语义和移动语义的区别:
一个对象如果可以在另一个位置被复制，则具有拷贝语义。
一个对象如果可以从一个变量移动到另一个变量，同时破坏性地修改原始对象，则具有移动语义
